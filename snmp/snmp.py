...
GSM_REG_FLAG = (0xf1, 0xf0, 0xf3, 0xf4)
...
EVENTS = {
    ...
    GSM_SMS_FLAG[0]: {1: ('channel', int),
                      ...
                      11: ('event', str),
                      'events': {0: 'SMS_INCOMING', 1: 'SMS_BROADCAST', 2: 'SMS_DELIVERED', '99': 'SMS_UNKNOWN'},
                      },
...
def parse_block(pack, header):
    ...
    if pack[pos] == '30':  # Флаг начала блока
        ...
        pos += TAIL_LEN
        if MIB[-1] > 6:
            TAIL = bytes.fromhex(''.join(TAIL)).decode()
        else:
            TAIL = ''.join(TAIL)
...
if __name__ == '__main__':
    ...
    for pk in sorted(glob.glob('pkt/*.bin')):
        with open(pk, 'rb') as pkf:
            addr, pack = pickle.loads(pkf.read())
            pack = wrap(pack.hex(), 2)
            pack1 = pack
            # print(f'{pk}')
            try:
